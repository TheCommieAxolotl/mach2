...
const runSimulation = (atomCount = 1000, probability = 0.01) => {
    ...
    class Atom extends mach2.Dynamic {
        ...
        public update() {
            ...
            const p = mach2.math.canvasToCartesian(this.ctx, this.x * this.ctx.canvas.width, this.y * this.ctx.canvas.height);

            mach2.draw.circle(this.ctx, p[0], p[1], 5, `hsl(${this.id * 360} ${this.id * 100}% ${this.id * 360}% / 50%)`);
        }
    }
        ...
        new (class extends mach2.Dynamic {
            public update() {
                ...
                mach2.graph.fn(
                    this.ctx,
                    () => {
                        prevY = mach2.math.lerp(prevY, this.ctx!.canvas.height, probability / Number(this.ctx?.canvas.dataset.resolution || "1"));
                ...
                this.ctx.fillText(`${(decayed / atomCount).toFixed(3)} / ${((atomCount - decayed) / atomCount).toFixed(3)}`, 32, 32);
