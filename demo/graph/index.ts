...
const latex2 = await mach2.latex.createLatexRenderingContext(
	scene.ctx,
	'\\Bigl(\\frac{\\pi}{4}, \\sqrt{2}\\Bigr)'
);
	...
	new (class extends mach2.Dynamic {
		...
		public async update() {
			...
			mach2.graph.fn(
				this.ctx,
				(x) => Math.sin(x),
				mach2.color.opacity(mach2.color.blue, this.opacities()[1]),
				4
			);
			mach2.graph.fn(
				this.ctx,
				(x) => Math.cos(x),
				mach2.color.opacity(mach2.color.red, this.opacities()[1]),
				4
			);
			...
			mach2.graph.fn(
				this.ctx,
				(x) => Math.sin(x) + Math.cos(x) * this.opacities()[0],
				mach2.color.opacity(mach2.color.green, this.opacities()[0]),
				4
			);
			...
			latex5.render(
				-4,
				1.1,
				mach2.color.opacity(mach2.color.lightBlue, this.opacities()[2]),
				'right',
				'top'
			);
			latex6.render(
				-4,
				-1.1,
				mach2.color.opacity(mach2.color.lightRed, this.opacities()[3]),
				this.positions().horizontal,
				'bottom'
			);
			latex.render(
				-4,
				1.5,
				mach2.color.opacity(mach2.color.lightGreen, this.opacities()[0]),
				'right',
				'top'
			);
		}
