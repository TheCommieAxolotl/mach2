...
import { lookAtTarget } from './render/plot';
...
export const scene3d = (
	...
	canvas.dataset.scene = SCENE_ID.toString() + 'three';
	...
	let cbs: Record<string, Parameters<Scene3D['on'][keyof Scene3D['on']]>[0][]> = {
		click: [] as Parameters<Scene3D['on'][keyof Scene3D['on']]>[0][]
	};
	...
	const ret: Scene3D = {
		...
		on: {
			click: (cb) => {
				cbs.click.push(cb as unknown as Parameters<Scene3D['on'][keyof Scene3D['on']]>[0]);
			}
		},
